...
DELIMITER $$
USE `OrderOnlineProcessing`$$
CREATE DEFINER = CURRENT_USER TRIGGER `OrderOnlineProcessing`.`ModifyBook` BEFORE UPDATE ON `Book` FOR EACH ROW
BEGIN
...
USE `OrderOnlineProcessing`$$
CREATE DEFINER = CURRENT_USER TRIGGER `OrderOnlineProcessing`.`PlaceOrder` AFTER UPDATE ON `Book` FOR EACH ROW
BEGIN
...
USE `OrderOnlineProcessing`$$
CREATE DEFINER = CURRENT_USER TRIGGER `OrderOnlineProcessing`.`ConfirmOrder` BEFORE DELETE ON `Order` FOR EACH ROW
BEGIN
